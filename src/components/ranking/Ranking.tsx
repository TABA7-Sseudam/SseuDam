...
export function Ranking() {
  ...
  useEffect(() => {
    ...
    const tooltipList = tooltipTriggerList.map(tooltipTriggerEl => new BootstrapTooltip(tooltipTriggerEl))
  ...
  const handleGuideClick = () => {
    navigate("/ranking/rank_tier_guide")
  }
    ...
    <div className="container mx-auto p-6 bg-white">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold flex items-center gap-2">
          ...
          <button 
            type="button" 
            className="btn btn-link p-0 ml-2 text-primary"  // 파란색 아이콘 적용
            onClick={handleGuideClick} 
            data-bs-toggle="tooltip" 
            data-bs-placement="right" 
            title="랭킹 & 등급 알아보기"
          >
            <i className="bi bi-question-circle-fill fs-2"></i>  {/* 크기 조정 (fs-2) */}
          </button>
        </h1>
